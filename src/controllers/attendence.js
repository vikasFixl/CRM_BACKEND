    ...
    try {
        ...
        if (!check) {
            ...
            for (var i = 0; i <= check.attendence.length - 1; i++) {
                if (check.attendence[i].date.toISOString().slice(0, 10) === req.body.date.slice(0, 10)) {
                    return res.json({ message: "Attendence already Marked for today" })
                }
            }
            const data = await attendenceModel.updateOne({ "eid": empid }, { $push: { attendence: req.body } })
            res.status(201).json({
                data: data,
                success: true,
                code: 201,
...
exports.markExit = async (req, res) => {
    try {
        ...
        for (var i = 0; i <= data.attendence.length - 1; i++) {
            //console.log(i);
            if (data.attendence[i].date.toISOString().slice(0, 10) === req.body.exit.slice(0, 10)) {
                //console.log("hwiqhro");
                // data.attendence[i].exit=req.body.exit
                // console.log(data.attendence[i].exit);
                const d = await attendenceModel.updateOne({ "attendence.exit": data.attendence[i].exit }, { $set: { "attendence.$.exit": req.body.exit } })
                break;
                //console.log(d);
            }
        }
...
exports.leave = async (req, res) => {
    try {
        ...
        //console.log(check);
            ...
            const data = await attendenceModel.updateOne({ "eid": empid }, { $push: { leaves: req.body } })
            res.status(201).json({
                data: data,
                success: true,
                code: 201,
    ...
    try {
        ...
        for (var i = 0; i <= data.leaves.length - 1; i++) {
            ...
            if (data.leaves[i].adminStatus === "Awaiting") {
                const up = await attendenceModel.updateOne({ "eid": empid,"leaves.adminStatus":data.leaves[i].adminStatus}, { $set: { "leaves.$.adminStatus": req.body.adminStatus } })
                return res.json({
                    data:up,
