...
export const startTrial = async (req, res) => {
    try {
        ...
        if(org.trials.includes(planId)){
            return res.status(400).json({ message: "Trial already used for this plan" });
        }
        const plan = await BillingPlan.findById(planId);
        if (!plan) return res.status(404).json({ message: "Billing plan not found" });

        if (plan.isFreePlan) {
            return res.status(400).json({ message: "Cannot start trial on a free plan" });
        }
        if (!plan.trial?.isTrialAvailable) {
            return res.status(400).json({ message: "Trial not available for this plan" });
        }
        ...
        org.trials.push(newBilling._id)
        await org.save();
